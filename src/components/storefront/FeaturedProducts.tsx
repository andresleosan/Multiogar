    ...
    <section className="bg-white py-14 dark:bg-slate-900 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            <p className="mb-2 inline-flex items-center gap-2 text-xs font-extrabold uppercase text-orange-600">
              <Flame className="h-4 w-4" /> Disponibilidad destacada
            </p>
            <h2 className="text-2xl font-black text-slate-950 dark:text-white sm:text-3xl">
              Productos listos para cotizar
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-400">
              Precios de referencia en USD. Selecciona una opción y confirma existencia con el equipo de ventas.
            </p>
          </div>
          ...
          <div
            ...
            className="flex max-w-full overflow-x-auto rounded-md border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-950"
          >
            {filters.map((filter) => (
              <button
                key={filter.value}
                ...
                className={`min-h-9 whitespace-nowrap rounded px-3 text-xs font-bold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-600 ${
        ...
        <div className="mt-12 flex flex-col justify-between gap-6 bg-blue-700 px-6 py-8 text-white sm:px-8 lg:flex-row lg:items-center">
          <div className="flex max-w-3xl items-start gap-4">
            <Building2 className="mt-1 h-7 w-7 shrink-0 text-orange-300" />
            <div>
          ...
          <Link
            href="/contacto"
            className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-md bg-orange-600 px-5 py-3 text-sm font-extrabold text-white transition-colors hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-white"
          >
